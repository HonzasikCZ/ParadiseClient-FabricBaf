        ...
        try {
            if (Objects.equals(channelName, "minecraft:register") || Objects.equals(channelName, "REGISTER")) // 1.13 channel or 1.8 channel
                for (String splitted : buf.toString(Charset.defaultCharset()).split("\000")) {
                    Helper.printChatMessage("&fChannel: &" + (ParadiseClient_Fabric.networkMod.getRegisteredChannelsByName().contains(splitted) ? "1" : "b") + " " + splitted);
                ...
                Helper.printChatMessage("&fChannel: &b" + channelName + " &fData: &b" + buf.toString(Charset.defaultCharset()));
        } catch (Exception e) {
